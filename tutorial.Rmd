...
```{r}
#remove rows with missing values and assign that data set to object
...
Another way to look at flipper length by species would be to use facets, which split the plot into panels. 
...
This function can be especially useful when there are many levels in one variable, and are trying to save space on your graphs. 
...
Based on these histograms, `facet_grid()` seems better suited to illustrate our data in a meaningful way. 
